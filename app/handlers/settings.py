...
def _s(lang: str, key: str, **kwargs) -> str:
    ...
    data = {
        "ru": {
            ...
            "balance_prompt":"💰 Теперь укажи стартовый баланс.\nПример: <code>15000</code>", "digits_only":"Нужны только цифры. Пример: <code>15000</code>", "no_accounts_alert":"Нет счетов",
        ...
        "en": {
            ...
            "balance_prompt":"💰 Now enter the starting balance.\nExample: <code>15000</code>", "digits_only":"Digits only. Example: <code>15000</code>", "no_accounts_alert":"No accounts",
        ...
        "kk": {
            ...
            "balance_prompt":"💰 Енді бастапқы балансты енгізіңіз.\nМысал: <code>15000</code>", "digits_only":"Тек цифрлар. Мысал: <code>15000</code>", "no_accounts_alert":"Шоттар жоқ",
...
async def _settings_root_text(db: aiosqlite.Connection, user_id: int, lang: str) -> str:
    ...
    lines = [
        ...
        _s(lang, "archived", value=archived_count),
...
def _notifs_text(lang: str, daily_enabled: int, daily_time: str, nudge_enabled: int, nudge_interval_min: int) -> str:
    ...
    return _s(lang, "notifs_tpl", daily=daily_status, time=(daily_time or '21:00'), nudge=nudge_status, hours=interval_hours)
...
async def _accounts_overview_text(db: aiosqlite.Connection, user_id: int, lang: str) -> str:
    ...
    archived_count = max(0, len(all_rows) - len(active))
    total_balance = sum(int(row[2] or 0) for row in active)

    lines = [
        _s(lang, "accounts_title"),
        "",
        _s(lang, "total", value=_fmt_money(total_balance)),
        _s(lang, "active", value=len(active)),
    ]
    ...
    for idx, (_acc_id, name, balance, _arch) in enumerate(active, start=1):
        lines.append(f"{idx}. {escape(str(name))} — <b>{_fmt_money(int(balance or 0))}</b>")

    richest = max(active, key=lambda row: int(row[2] or 0))
    lines.append("")
    lines.append(({"ru": "💡 Самый крупный счёт", "en": "💡 Largest account", "kk": "💡 Ең үлкен шот"}.get(lang, "💡 Самый крупный счёт")) + f": <b>{escape(str(richest[1]))}</b> — <b>{_fmt_money(int(richest[2] or 0))}</b>")
...
async def _archived_accounts_text(db: aiosqlite.Connection, user_id: int, lang: str) -> str:
    ...
    for idx, (_acc_id, name, balance, _arch) in enumerate(archived, start=1):
        lines.append(f"{idx}. {escape(str(name))} — <b>{_fmt_money(int(balance or 0))}</b>")
    lines.append("")
    lines.append({
...
async def _account_card_text(db: aiosqlite.Connection, user_id: int, account_id: int, lang: str) -> str:
    ...
    _acc_id, name, balance, is_archived = acc
    status = {
    ...
    lines = [
        ...
        "",
        ({"ru": "💰 Баланс", "en": "💰 Balance", "kk": "💰 Баланс"}.get(lang, "💰 Баланс")) + f": <b>{_fmt_money(int(balance or 0))}</b>",
        ({"ru": "📌 Статус", "en": "📌 Status", "kk": "📌 Күйі"}.get(lang, "📌 Статус")) + f": <b>{status}</b>",
        "",
        {
            "ru": "Здесь можно быстро переименовать счёт, поправить баланс или убрать его в архив.",
            "en": "Here you can quickly rename the account, adjust its balance or archive it.",
            "kk": "Мұнда шот атауын өзгертуге, балансын түзетуге немесе архивке жіберуге болады.",
        }.get(lang, "Здесь можно быстро переименовать счёт, поправить баланс или убрать его в архив."),
    ]
    return "\n".join(lines)
...
async def _go_notifs_menu(
    ...
    await _render_screen(
        target,
        state,
        _notifs_text(await get_lang(db, target.from_user.id), daily_enabled, daily_time, nudge_enabled, nudge_interval),
        reply_markup=notifications_kb(
            daily_enabled,
            daily_time,
            nudge_enabled,
            nudge_interval,
            await get_lang(db, target.from_user.id),
        ),
    )
...
async def st_acc_add_balance(m: Message, state: FSMContext, db: aiosqlite.Connection):
    ...
    if not raw.isdigit():
        await m.answer(_s((await state.get_data()).get("lang", "ru"), "digits_only"), parse_mode=PARSE_MODE)
        return
    ...
    try:
        _account_id, action = await create_account(db, m.from_user.id, name, balance, now_iso())
        await db.commit()
        ...
        if str(e) == "active_name_exists":
            msg = {
                ...
                "kk": "Осындай атаумен белсенді шот already exists.",
            }.get((await state.get_data()).get("lang", "ru"), "Счёт с таким названием уже существует среди активных.")
    ...
    if action == "restored":
        ...
        msg = {
            "ru": "♻️ <b>Архивный счёт восстановлен</b>\n\nНовый счёт с таким именем не создавался — бот вернул существующий архивный счёт и обновил его баланс.",
            "en": "♻️ <b>Archived account restored</b>\n\nA new account with that name was not created — the archived one was restored and its balance was updated.",
            "kk": "♻️ <b>Архивтегі шот қалпына келтірілді</b>\n\nОсындай атаумен жаңа шот құрылмады — архивтегі шот қайтарылып, балансы жаңартылды.",
        }.get(lang)
        await m.answer(msg, parse_mode=PARSE_MODE)
...
async def st_notifs_daily_time(c: CallbackQuery, state: FSMContext):
    await _enter_input_mode(
        c,
        state,
        screen_text="⏰ <b>Время ежедневного отчёта</b>\n\nУкажи время, когда бот должен отправлять итог дня.",
        prompt_text=_s(await get_lang(db, c.from_user.id), "time_example"),
