...
async def login_command(m: Message, state: FSMContext, db: aiosqlite.Connection):
    ...
    try:
        ...
        # Fetch username and password_hash from DB using Telegram ID (which is user_id)
        async with db.execute("SELECT username, password_hash FROM users WHERE id = ?", (user_id,)) as cursor:
        ...
        username, password_hash = user_row[0], user_row[1]
        
        if not password_hash or password_hash == 'LEGACY_PLACEHOLDER':
            await db.execute("UPDATE users SET onboarding_state = 'waiting_legacy_password' WHERE id = ?", (user_id,))
            await db.commit()
        ...
        if lang == "kk":
            ...
            btn_change_pwd = "✏️ Изменить пароль"
...
def _upgrade_message(lang: str, has_full_access: bool = False, promo_used: bool = False) -> str:
    if lang == "en":
        ...
        price_3m = "150 ⭐"
        return (
            "👑 <b>Upgrade your financial management with Premium</b>\n\n"
            f"{greeting}\n\n"
            ...
            "Investing in financial discipline is an investment that pays back.\n\n"
            "👇 Press one of the buttons below to unlock all features now!"
        )
    ...
    if lang == "kk":
        ...
        price_3m = "150 ⭐"
        return (
            "👑 <b>Premium-мен қаржыңызды басқаруды жаңа деңгейге көтеріңіз</b>\n\n"
            f"{greeting}\n\n"
    ...
    price_3m = "150 ⭐"
    return (
        "👑 <b>Переход на новый уровень управления деньгами с Premium</b>\n\n"
        f"{greeting}\n\n"
