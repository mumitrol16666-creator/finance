...
- Этап 3: Пользователь рассказал про свои цели/мечты.
  Твоя задача: Поддержать его цели/мечту с эмпатией и связать её с реальностью. Спроси про Этап 4 (установление дневного лимита), подстраивая вопрос под его мечту (например: если мечта — путешествия или покупка машины, укажи, что лимит поможет откладывать на это и не тратить лишнего на повседневные нужды вроде еды, такси или мелочей). Сформулируй вопрос так: "Какая сумма дневного лимита на повседневные расходы будет для тебя комфортной? Напиши просто число."
...
def get_fallback_interview_response(stage: int, user_answer: str, data: dict, lang: str = "ru") -> str:
    ...
    if stage == 1:
        ...
        dream_val = user_answer.strip() if user_answer else ""
        if len(dream_val) > 50:
            dream_val = dream_val[:47] + "..."
        comments = {
            "ru": f"Мечта «{dream_val}» — это отличный ориентир! Деньги — это инструмент для достижения того, что действительно приносит радость и развитие.",
            "en": f"The dream of '{dream_val}' is a great guide! Money is a tool to achieve what really brings joy and development.",
            "kk": f"«{dream_val}» арманы – тамаша бағыт! Ақша — бұл шынайы қуаныш пен даму әкелетін нәрселерге қол жеткізу құралы."
        }
        questions = {
            "ru": "А теперь давай вернемся к реальности. Чтобы уверенно идти к ней и контролировать спонтанные траты, давай установим твой комфортный дневной лимит расходов на повседневные нужды (продукты, такси, развлечения). Какую сумму в день ты готов(а) тратить в обычные дни? Напиши просто число.",
            "en": "Now let's return to reality. To confidently move towards it and control spontaneous spending, let's set a comfortable daily limit for everyday expenses (groceries, transport, fun). How much are you ready to spend per day? Just type the number.",
            "kk": "Енді шындыққа оралайық. Оған сенімді түрде жету және кездейсоқ шығындарды бақылау үшін күнделікті шығындарға (азық-түлік, такси, ойын-сауық) ыңғайлы күндік лимит белгілейік. Күніне қанша жұмсауға дайынсыз? Тек санды жазыңыз."
        }
        comment = comments.get(lang, comments["ru"])
        question = questions.get(lang, questions["ru"])
        return f"{comment}\n\n{question}"
