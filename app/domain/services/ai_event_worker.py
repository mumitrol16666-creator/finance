...
async def analyze_and_update_profile(db: aiosqlite.Connection, user_id: int) -> None:
    ...
    # Evaluate the stricter stage first. Previously the budgeting branch
    # shadowed investing, making investing unreachable for healthy profiles.
    ...
    summary_text = "Пользователь " + (
        ", ".join(summaries)
        if summaries
        else "ведёт стабильный и сбалансированный бюджет"
    )
