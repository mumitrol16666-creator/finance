...
async def upsert_user(db: aiosqlite.Connection, user_id: int, created_at: str):
    """
    Upserts a user when they first start interacting with the bot.
    Sets default fields and onboarding_state to 'select_auth_type'.
    """
    await db.execute(
        "INSERT OR IGNORE INTO users("
        "  id, telegram_id, username, password_hash, display_name, "
        "  onboarding_state, created_at, onboarded"
        ") VALUES(?,?,?,?,?,?,?,0)",
        (
            user_id, 
            user_id, 
...
async def set_onboarded(db: aiosqlite.Connection, user_id: int, onboarded: int):
    await db.execute("UPDATE users SET onboarded=? WHERE id=?", (onboarded, user_id))

async def get_onboarded(db: aiosqlite.Connection, user_id: int) -> int | None:
    cur = await db.execute("SELECT onboarded FROM users WHERE id=?", (user_id,))
...
async def get_streak(db: aiosqlite.Connection, user_id: int) -> tuple[int, int, str | None]:
    cur = await db.execute(
        "SELECT COALESCE(current_streak,0), COALESCE(max_streak,0), last_activity_date FROM users WHERE id=?",
        (user_id,),
    )
    row = await cur.fetchone()
    if not row:
...
async def get_access_profile(db: aiosqlite.Connection, user_id: int):
    cur = await db.execute(
        "SELECT onboarded, current_streak, max_streak, last_activity_date, COALESCE(mode, 'newbie') AS mode, COALESCE(progress_level,0) AS progress_level, COALESCE(full_access,0) AS full_access, full_access_until FROM users WHERE id=?",
        (user_id,),
    )
    return await cur.fetchone()

async def set_progress_level(db: aiosqlite.Connection, user_id: int, progress_level: int):
    await db.execute("UPDATE users SET progress_level=? WHERE id=?", (int(progress_level), user_id))

async def set_mode(db: aiosqlite.Connection, user_id: int, mode: str):
    await db.execute("UPDATE users SET mode=? WHERE id=?", (str(mode or 'newbie').lower(), user_id))

async def grant_full_access(db: aiosqlite.Connection, user_id: int, days: int = 90):
    from datetime import datetime, timedelta, timezone, date
    
    # Get current state
    cur = await db.execute("SELECT full_access_until FROM users WHERE id=?", (user_id,))
    row = await cur.fetchone()
...
async def set_newbie_defaults(db: aiosqlite.Connection, user_id: int):
    cur = await db.execute("SELECT full_access FROM users WHERE id=?", (user_id,))
    row = await cur.fetchone()
    if row and int(row[0] or 0) == 1:
        # Keep full access mode and active subscription, only reset progress level
        await db.execute("UPDATE users SET progress_level=0 WHERE id=?", (user_id,))
    else:
        await db.execute("UPDATE users SET mode='newbie', progress_level=0, full_access=0 WHERE id=?", (user_id,))

async def get_free_exports_used(db: aiosqlite.Connection, user_id: int) -> int:
    cur = await db.execute("SELECT COALESCE(free_exports_used, 0) FROM users WHERE id = ?", (user_id,))
    row = await cur.fetchone()
    return int(row[0]) if row else 0

async def increment_free_export(db: aiosqlite.Connection, user_id: int) -> None:
    await db.execute(
        "UPDATE users SET free_exports_used = COALESCE(free_exports_used, 0) + 1 WHERE id = ?",
        (user_id,),
    )

async def get_all_active_users(db: aiosqlite.Connection) -> list[int]:
    cur = await db.execute("SELECT id FROM users WHERE onboarded = 1")
    rows = await cur.fetchall()
    return [row[0] for row in rows]

async def is_promo_used(db: aiosqlite.Connection, user_id: int) -> bool:
    cur = await db.execute("SELECT COALESCE(promo_used, 0) FROM users WHERE id = ?", (user_id,))
    row = await cur.fetchone()
    return int(row[0]) == 1 if row else False

async def mark_promo_used(db: aiosqlite.Connection, user_id: int) -> None:
    await db.execute("UPDATE users SET promo_used = 1 WHERE id = ?", (user_id,))
...
async def mark_trial_3d_claimed(db: aiosqlite.Connection, user_id: int) -> None:
    await db.execute("UPDATE users SET trial_3d_claimed = 1 WHERE id = ?", (user_id,))
