...
*   **Runtime:** Python 3.14 with `asyncio`
...
2.  **Environment Configuration:**
    ...
    ```
    Generate the signing key with `openssl rand -hex 32` and paste the result
    after `SECRET_KEY=`. Never commit the populated `.env` file.
3.  **Run the bot:**
    ```bash
